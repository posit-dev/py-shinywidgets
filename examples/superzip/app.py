...
from shiny import *
from ipyshiny import output_ipywidget, render_ipywidget
...
ui_map = ui.TagList(
  output_ipywidget("map", width="100%", height="100%"),
  ui.panel_fixed(
    ...
    id="controls", class_="panel panel-default",
    width="330px", height="auto", draggable=True, 
    top="60px", left="auto", right="20px", bottom="auto",
  ),
  ui.div(
    'Data compiled for ', 
    ...
    ui.nav(
      ...
      ui.row(
        ...
        ui.column(3,
          ui.panel_conditional(
            "input.states",
            ui.input_select("cities", "Cities", {"": "All cities"}, multiple=True)
          )
        ),
        ui.column(3,
          ui.panel_conditional(
            "input.states",
            ui.input_select("zipcodes", "Zipcodes", {"": "All zipcodes"}, multiple=True)
          )
        )
      ),
      ui.row(
        ui.column(1,
          ui.input_numeric("minScore", "Min score", min=0, max=100, value=0)
        ),
        ui.column(1,
          ui.input_numeric("maxScore", "Max score", min=0, max=100, value=100)
        )
      ),
      ui.hr(),
      output_ipywidget("data")
    ),
    title="Superzip"
)

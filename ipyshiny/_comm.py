...
class ShinyComm:
    ...
    _msg_callback: Optional[Callable[[Dict], None]] = None
    _close_callback: Optional[Callable[[Dict], None]] = None
    _closed: bool = False
    _closed_data: Dict = {}

    def __init__(self, comm_id: str, comm_manager: ShinyCommManager, target_name: str, data=None, metadata=None, buffers=None, **keys) -> None:
        self.comm_id = comm_id
        self.comm_manager = comm_manager
        self.target_name = target_name
        self.open(data=data, metadata=metadata, buffers=buffers, **keys)

    def open(self, data=None, metadata=None, buffers=None, **keys) -> None:
        self.comm_manager.register_comm(self)
        try:
            self._publish_msg(
                'ipyshiny_comm_open', data=data, metadata=metadata, buffers=buffers,
                target_name=self.target_name, target_module=None, **keys
            )
            self._closed = False
    ...
    def send(self, data=None, metadata=None, buffers=None) -> None:
        self._publish_msg("ipyshiny_comm_msg", data=data, metadata=metadata, buffers=buffers)
    ...
    def close(self, data=None, metadata=None, buffers=None, deleting=False) -> None:
        ...
        self._publish_msg("ipyshiny_comm_close", data=data, metadata=metadata, buffers=buffers)
        if not deleting:
    ...
    def _publish_msg(self, msg_type: str, data=None, metadata=None, buffers=None, **keys) -> None:
        ...
        if session is None:
            raise RuntimeError("Cannot send an ipywidget messages to the client outside of a Shiny session context.")
        ...
        def _send():
            run_coro_sync(session.send_custom_message(msg_type, msg_txt))
